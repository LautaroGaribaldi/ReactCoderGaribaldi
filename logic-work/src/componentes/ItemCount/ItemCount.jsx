  ...
  return (
    <div style={{marginBottom:"10px"}}>
      <div style={{display:"flex", flexDirection:"row", flexWrap:"wrap", justifyContent:"center", marginBottom:"10px", alignItems:"center"}}>
        <button className= {`btn btn-primary ${(stock !== 0 )? "": "disabled"}`} onClick={sumar} >
            +
        </button>
        <label>{count}</label>
        <button className= {`btn btn-primary ${(stock !== 0 )? "": "disabled"}`} onClick={restar}>
            -
        </button>
      </div>
        <div>
          <Button style={{marginBottom:"10px"}} variant="primary" className={(stock !== 0 )? "": "disabled"} onClick={agregarCarrito}>
